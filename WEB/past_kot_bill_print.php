...
$salesId="";
 ...
 if((isset($salesId)) && (isset($table)) && (isset($chair))){
...
$print = mysql_query("select kop_print from printer");
if (mysql_num_rows($print) > 0) {

    while ($rowsprint=mysql_fetch_array($print)){
//echo $rowsprint["kop_print"]; 
...
$stpastdetails = mysql_query("select item,qty,itm_code,remarks from past_bill_details where date='$curnt_date' and tables='$table' and chair='$chair' and bill_no='0' and mob_status ='0' and sales_id='$salesId' ");
if (mysql_num_rows($stpastdetails) > 0) {
    $items["items"] = array();

    while ($rowspast=mysql_fetch_array($stpastdetails)){
...
// echo no users JSON
        ...
        $stKOTs = mysql_query("update past_bill_details set mob_status=1 where tables='$table' and chair='$chair'and bill_no=0" );
