...
$tes = json_decode(file_get_contents('php://input', true));
...
if (isset($tes)) {
    ...
    foreach ($tes as $obj) {
        ...
        foreach ($obj as $key => $val) {
          ...
          if($key == "unit_type"){
          $unitTypesquery= mysqli_query($con, "SELECT i.*,u.price,p.short_name FROM `item_master` i left join unitwise_price_line u on u.icode=i.itm_code left join prod_unit p on p.uid=u.unit WHERE i.itm_code='$val'");
          if ($unitTypesquery) {
        ...
        //INSERT INTO past_bill_details (`bill_no`,`itm_code`,`item`,`unit`,`qty`,`remarks`,`amt`,`date`,`time`,`sales_id`,`e_id`,`shift`,`tables`,`chair`,`unit_type`,`ser_tax`,`vat_tax`,`service`,`vat`,`tot_amt`,`cat`,`mob_status` ) VALUES('0','$itm_code','$item','$amount','$qty','$remark','$totlAmt','$date','$time','$sales','$empId','$shift','$bill_no','$chair','$unitType','$ser_tax','$vat','$service','$vat_tax','$totlAmtfi','0',0)
        $insertKeys = '`' . implode('`,`', array_keys($insertPairs)) . '`';
        $insertVals = '"' . implode('","', array_values($insertPairs)) . '"';

       // echo "INSERT INTO past_bill_details  ({$insertKeys}) VALUES ({$insertVals})" . "\n";

        $result = mysqli_query($con,"INSERT INTO past_bill_details  ({$insertKeys}) VALUES ({$insertVals})");
