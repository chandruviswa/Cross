...
if(isset($_POST['delete'])){
    ...
    $itemQuery=mysqli_query($con,"DELETE FROM `products`  WHERE `product_id` ='".$_POST['delete']."'");
    if($itemQuery){
        echo "<script>alert('Successfully deleted')</script>";
        //$nes="Employee deleted";
        //$createDate=date("Y-m-d : H:i:s");
        //$newsQuery=mysqli_query($con,"DELETE FROM `products`  WHERE `product_id` ='".$_POST['delete']."' AND sid='".$_SESSION['sid']."' AND aid='".$_SESSION['aid']."'");
    }
}
...
if(isset($_POST['edit'])){
    $sales_id=$_POST['edit'];
    header("Location:addproduct.php?edit=".$sales_id."");
}
...
<!--    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>-->
    <script type="text/javascript">
        ...
        function deleteEmp(id){
            $.ajax({
                url: 'ajaxdelete.php?deleteproduct='+id, //call storeemdata.php to store form data
        ...
        <div class="main">
            ...
            <div class="main-content">
                <div class="container-fluid">
                    <h3  style="color: #066ECC ;" >Product Management  <button name="addemp" class="btn btn-success">Add Product</button></h3>
                                ...
                                <div class="panel-body">
                                    <div style="overflow-x:auto;">
                                        <table>
                                            ...
                                            $SelectLeadQuery=mysqli_query($con,"SELECT * FROM  `products`");
                                            $Sno=0;
                                            while($arraylead=mysqli_fetch_array($SelectLeadQuery)){
                                                ...
                                                <tr>
                                                    ...
                                                    <td><a href="addproduct.php?edit=<?php if(isset($arraylead['product_id'])){echo $arraylead['product_id'];}?>"><?php if(isset($arraylead['product'])){echo $arraylead['product'];}?></a></td>
                                                    <td><a href="addproduct.php?edit=<?php if(isset($arraylead['product_id'])){echo $arraylead['product_id'];}?>"><?php if(isset($arraylead['description'])){echo $arraylead['description'];}?></a></td>

                                                    <!--                                                    <td><button type="submit" name="view" value="--><?php //if(isset($arraylead['id'])){echo $arraylead['id'];}?><!--" class="btn btn-success">View</button></td>-->
                                                    <td><img onclick="deleteEmp(<?php if(isset($arraylead['product_id'])){echo $arraylead['product_id'];}?>)" style="width: 20px;height: 20px;margin-left: 10px;margin-right: 10px"  src="assets/img/close.png"></td>
